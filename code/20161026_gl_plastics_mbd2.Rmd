...
meanplot <- ggplot(
  ...
  aes(x = size_fraction, y = mean_station_concentration))
meanplot + 
...
* Zero values (n = 23) removed for some reason, I can't figure out how to fix it right now
...
sdplot <- ggplot(data = gl.2014.conc.melt.sd.df, aes(x = size_fraction, y = sd_in_station_concentration))
sdplot + 
...
* Zero values (n = 22) removed for some reason, I can't figure out how to fix it right now
...
seplot <- ggplot(data = gl.2014.conc.melt.se.df, aes(x = size_fraction, y = se_in_station_concentration))
...
varplot <- ggplot(data = gl.2014.conc.melt.var.df, aes(x = size_fraction, y = variance_in_station_concentration))
varplot + 
  geom_boxplot(aes(fill = size_fraction, alpha = .1)) + 
...
<!--- ## Correlogram of numerical metadata and concentrations, size class totals --->

<!--- ```{r plot_gl_2014_corrgram_concentrations_by_size_metadata_numeric_only}

## corrgram only uses numeric variables, so I pull those out into their own dataframe first

gl.2014.conc.by.size.metadata.numeric.only<- gl.2014.conc.by.size.metadata[sapply(gl.2014.conc.by.size.metadata, is.numeric)]

head(gl.2014.conc.by.size.metadata.numeric.only)

## Create a corrgram with shade and pie charts

gl_2014_by_size_numeric_corrgram_lab <- as.character(colnames(gl.2014.conc.by.size.metadata.numeric.only))


corrg_label_numbers_v2 <- c(1:length(gl_2014_by_size_numeric_corrgram_lab))

corrgram.gl.2014.conc.by.size.metadata.num.only <- corrgram(gl.2014.conc.by.size.metadata.numeric.only, 
  lower.panel=panel.shade,
  upper.panel=panel.pie, 
  text.panel=panel.txt,
  labels = corrg_label_numbers_v2,
  cex.labels = 1
  )
...
<!--- ```{r table_gl_2014_corrgram_by_size_label_legend}

table.corrg.by.size.nums.labs <- data.frame(Number = corrg_label_numbers_v2, Variable = gl_2014_by_size_numeric_corrgram_lab)

kable(table.corrg.by.size.nums.labs, format = "markdown")
``` 
--->
...
gl.all.data.nonnumeric.long <- melt(gl.2014.all.data.nonnumeric, value.vars = c('fragment_conc_km2', 'film_conc_km2', 'fiber_conc_km2', 'filament_conc_km2', 'nurdle_conc_km2', 'sphere_conc_km2', 'foam_conc_km2', 'paint_conc_km2', 'total_conc_km2'), value.name = "concentration", variable.name = "plastic_type")

View(gl.all.data.nonnumeric.long)
```
...
ggplot(data = gl.all.data.nonnumeric.long) +
  geom_jitter(aes(x = size_fraction_um, 
                  y = concentration, 
                  #shape = size_fraction_um, 
                  color = station_class, 
                  fill = station_class), subset = .(plastic_type != 'trawl_start_time')) +
  scale_y_log10(breaks = c(100, 1000, 10000, 100000, 100000, 1000000)) + 
